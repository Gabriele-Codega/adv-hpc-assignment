...
int main(int argc, char* argv[]){
    ...
    MPI_Aint byte_dimension = sizeof(double) * ( n_loc + 2 ) * ( dimension + 2 );
    MPI_Alloc_mem(byte_dimension,MPI_INFO_NULL,&matrix);
    MPI_Alloc_mem(byte_dimension,MPI_INFO_NULL,&matrix_new);
    ...
    {
	MPI_Win_create( &matrix[dimension+2], (dimension+2)*sizeof(double), sizeof(double), MPI_INFO_NULL,MPI_COMM_WORLD,&top_win);
	MPI_Win_create( &matrix[n_loc*(dimension+2)], (dimension+2)*sizeof(double), sizeof(double), MPI_INFO_NULL,MPI_COMM_WORLD,&bot_win);
    }
    // start algorithm
	...
	#pragma acc host_data use_device(matrix)
	{
	    ...
	    MPI_Win_fence(0,bot_win);
	    MPI_Get(&matrix[(n_loc+1)*(dimension+2)], (MPI_Aint)(dimension+2), MPI_DOUBLE, \
		    next, 0, (MPI_Aint)(dimension+2), MPI_DOUBLE, top_win);

	    MPI_Get(matrix, (MPI_Aint)(dimension+2), MPI_DOUBLE, \
		    prev, 0, (MPI_Aint)(dimension+2),MPI_DOUBLE, bot_win);
	    MPI_Win_fence(0,top_win);
	    MPI_Win_fence(0,bot_win);
	}
